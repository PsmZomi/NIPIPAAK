...
function withoutArchive(folders) {
  return (folders || []).filter(
    (f) => f?.name && f.name.toLowerCase() !== 'archive',
  );
}
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const cleaned = withoutArchive(JSON.parse(saved));
        setFolders(cleaned);
        localStorage.setItem('galleryFolders', JSON.stringify(cleaned));
        if (cleaned.length > 0) setCurrentFolderId(cleaned[0].id);
      } catch {
        setFolders([]);
      }
    } else {
  ...
  return (
    <main className="min-h-screen bg-[#fafafa] pt-[64px] sm:pt-[72px] lg:pt-[89px] pb-24 lg:pb-12">
      <div className="max-w-7xl mx-auto px-5 relative">
        <header className="text-center mb-8">
          <h1
            className="text-5xl font-display mt-4 text-ink mb-2"
          ...
          {user && (
            <div className="flex items-center bg-white border border-zinc-200 rounded-full px-4 py-1">
              ...
              <button
                type="button"
                onClick={addFolder}
                className="text-green-600 font-bold px-2 text-lg"
              >
                +
              </button>
            </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-pulse">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className="aspect-square bg-zinc-200 rounded-lg" />
            ))}
          </div>
        ) : images.length === 0 ? (
          <p className="text-center font-mono text-xs text-zinc-400 py-16">
            {currentFolder
              ? 'No photos in this album yet.'
              : 'Photos appear here after you create an album and upload.'}
          </p>
        ) : (
